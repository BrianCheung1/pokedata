...
const renderMoves = (moves?: Move[], charged?: boolean) => {
  ...
  <Table.ScrollContainer minWidth={500}>
    <Table horizontalSpacing="md" captionSide="bottom" highlightOnHover>
      ...
      <Table.Thead>
        <Table.Tr>
          <Table.Th>
            {moves?.elite_fast_moves &&
              moves?.elite_fast_moves?.length > 0 &&
              "Elite Fast Moves"}
          </Table.Th>
        </Table.Tr>
      </Table.Thead>
      <Table.Tbody>{renderMoves(moves?.elite_fast_moves)}</Table.Tbody>
      <Table.Thead>
        <Table.Tr>
          <Table.Th>Charged Moves</Table.Th>
        </Table.Tr>
      </Table.Thead>
      <Table.Tbody>{renderMoves(moves?.charged_moves, true)}</Table.Tbody>
      <Table.Thead>
        <Table.Tr>
          <Table.Th>
            {moves?.elite_charged_moves &&
